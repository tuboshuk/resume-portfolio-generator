...
import { getAllLearningTags, learning } from "@/lib/content";

export function LearningClient() {
  const [query, setQuery] = useState("");
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
  const allTags = useMemo(() => getAllLearningTags(), []);

  const filtered = useMemo(() => {
    const out = filterLearning({ notes: learning, query, selectedTags });
    return out.sort((a, b) => {
    ...
    <main>
      <section className="border-b border-white/10">
        <Container className="py-12 sm:py-14">
          <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">学习</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-white/65 sm:text-base">
            用来沉淀学习笔记、面试题与成长记录。支持搜索与标签筛选，构建后可导出为纯静态站点。
          </p>
      ...
      <section>
        <Container className="py-12 sm:py-14">
          <div className="flex items-center justify-between gap-6">
            <div className="text-sm text-white/65">共 {filtered.length} 条记录</div>
          </div>

          {filtered.length === 0 ? (
            <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-10 text-sm text-white/70">
              没有匹配的记录，试试清除筛选或修改搜索词。
            </div>
          ) : (
            <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((n) => (
                <LearningCard key={n.slug} note={n} />
              ))}
            </div>
          )}
        </Container>
